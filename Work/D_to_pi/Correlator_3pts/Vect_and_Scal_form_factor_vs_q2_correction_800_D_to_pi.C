...
string beta_V_2[5] =  {"1.90_32", "1.90_24", "1.95_32", "1.95_24", "2.10_48"};
...
int main(){
  ...
  for(int ith1 = 0; ith1 < Nth1; ith1++){
	...
	sprintf(open_fzero, "OUTPUT_SMEAR/%s/%s/fzero_corrected/%s/%s/%s/fzero_corrected.%s.m1m2_%sc.th_%s%s.out", dir_S[iS].c_str(), dir_E[iE].c_str(), strategy[istrategy].c_str(), beta_V[ibeta].c_str(), mu_sea_1[ibeta][imusea].c_str(), mu_sea_2[imusea].c_str(), m[0].c_str(), th_[ith1].c_str(), th_[ith2].c_str());
	sprintf(open_fplus, "OUTPUT_SMEAR/%s/%s/fplus_corrected/%s/%s/%s/fplus_corrected.%s.m1m2_%sc.th_%s%s.out", dir_S[iS].c_str(), dir_E[iE].c_str(), strategy[istrategy].c_str(), beta_V[ibeta].c_str(), mu_sea_1[ibeta][imusea].c_str(), mu_sea_2[imusea].c_str(), m[0].c_str(), th_[ith1].c_str(), th_[ith2].c_str());
	
      }else if(no_correction == 1){
	
	sprintf(open_fzero, "OUTPUT_SMEAR/%s/%s/fzero_corrected/%s/%s/%s/fzero_no_corrected.%s.m1m2_%sc.th_%s%s.out", dir_S[iS].c_str(), dir_E[iE].c_str(), strategy[istrategy].c_str(), beta_V[ibeta].c_str(), mu_sea_1[ibeta][imusea].c_str(), mu_sea_2[imusea].c_str(), m[0].c_str(), th_[ith1].c_str(), th_[ith2].c_str());
	sprintf(open_fplus, "OUTPUT_SMEAR/%s/%s/fplus_corrected/%s/%s/%s/fplus_no_corrected.%s.m1m2_%sc.th_%s%s.out", dir_S[iS].c_str(), dir_E[iE].c_str(), strategy[istrategy].c_str(), beta_V[ibeta].c_str(), mu_sea_1[ibeta][imusea].c_str(), mu_sea_2[imusea].c_str(), m[0].c_str(), th_[ith1].c_str(), th_[ith2].c_str());
	
      }
	
      if ((fr_fzero = fopen(open_fzero, "r")) == NULL ){
	printf("Error opening the input file: open_fzero\n");
	exit(EXIT_FAILURE);
      }
      if(ith1 != 3 || ith2 != 3){
	if ((fr_fplus = fopen(open_fplus, "r")) == NULL ){
	  printf("Error opening the input file: open_fplus\n");
	  exit(EXIT_FAILURE);
	}
      }
  ...
  if(no_correction == 0){
    ...
    sprintf(file_out_fzero, "OUTPUT_SMEAR/%s/%s/fzero_corrected/%s/fzero_vs_q2/fzero_corrected_vs_q2.%s.%s.m1m2_%sc.out", dir_S[iS].c_str(), dir_E[iE].c_str(), strategy[istrategy].c_str(), beta_V_2[ibeta].c_str(), mu_sea_2[imusea].c_str(), m[0].c_str());
    sprintf(file_out_fplus, "OUTPUT_SMEAR/%s/%s/fplus_corrected/%s/fplus_vs_q2/fplus_corrected_vs_q2.%s.%s.m1m2_%sc.out", dir_S[iS].c_str(), dir_E[iE].c_str(), strategy[istrategy].c_str(), beta_V_2[ibeta].c_str(), mu_sea_2[imusea].c_str(), m[0].c_str());
    
  } else if(no_correction == 1){
    
    sprintf(file_out_fzero, "OUTPUT_SMEAR/%s/%s/fzero_corrected/%s/fzero_vs_q2/fzero_no_corrected_vs_q2.%s.%s.m1m2_%sc.out", dir_S[iS].c_str(), dir_E[iE].c_str(), strategy[istrategy].c_str(), beta_V_2[ibeta].c_str(), mu_sea_2[imusea].c_str(), m[0].c_str());
    sprintf(file_out_fplus, "OUTPUT_SMEAR/%s/%s/fplus_corrected/%s/fplus_vs_q2/fplus_no_corrected_vs_q2.%s.%s.m1m2_%sc.out", dir_S[iS].c_str(), dir_E[iE].c_str(), strategy[istrategy].c_str(), beta_V_2[ibeta].c_str(), mu_sea_2[imusea].c_str(), m[0].c_str());
    
  }
  
  if ((fout_fzero = fopen(file_out_fzero, "w")) == NULL ){
    printf("Error opening the input file: file_out_fzero\n");
    exit(EXIT_FAILURE);
  }
  if ((fout_fplus = fopen(file_out_fplus, "w")) == NULL ){
    printf("Error opening the input file: file_out_fplus\n");
    exit(EXIT_FAILURE);
  }
