...
void Vckm_from_sqrdGamma_boot( double*, int, int, double*, double*, double [Nev_tot+1][npar], double*, int, double (*)( double, void*));
void Vckm_from_fpVckm_boot( double*, int, int, double*, double [Nev_tot+1][npar], double*, double (*)( double, void*));
...
int main(){
  ...
  *var_f0p = 5.0*pow(10, -6);
  ...
  //num_random_multivariata_5par( mu, sigma, m_par, npar, par_sint[0], par_sint[1], par_sint[2], par_sint[3], par_sint[4], Nrand);
  //num_random_multivariata_5par( par_boot[0], sigma, boot_cov_par, npar, par_sint[0], par_sint[1], par_sint[2], par_sint[3], par_sint[4], Nrand);
	num_random_multivariata_5par( mu, sigma, boot_cov_par, npar, par_sint[0], par_sint[1], par_sint[2], par_sint[3], par_sint[4], Nrand);
  ...
  for(int iq = 0; iq < Nstep; iq++){
    ...
    for(int iev = 1; iev <= Nev_sint; iev++){
			...
			params.res        = par_sint_2[iev-1][0];
			params.A_fzero    = par_sint_2[iev-1][1];
			params.A_fplus    = par_sint_2[iev-1][2];
			params.polo_fzero = par_sint_2[iev-1][3];
			params.polo_fplus = par_sint_2[iev-1][4];
  ...
  // BABAR
	Vckm_from_sqrdGamma_boot( Vcd_BaBar, Nev_tot, NBaBar, q2_inf_BaBar, q2_sup_BaBar, par_boot, sqr_dGamma_BaBar, 0, p_cube);

  // CLEO-D0
	Vckm_from_sqrdGamma_boot( Vcd_Cleo_D0, Nev_tot, NCleo_D0, q2_inf_Cleo_D0, q2_sup_Cleo_D0, par_boot, sqr_dGamma_Cleo_D0, 0, p_cube);

  // CLEO-D+
	Vckm_from_sqrdGamma_boot( Vcd_Cleo_Dp, Nev_tot, NCleo_Dp, q2_inf_Cleo_Dp, q2_sup_Cleo_Dp, par_boot, sqr_dGamma_Cleo_Dp, 1, p_cube);

  // BES3-D0
	Vckm_from_sqrdGamma_boot( Vcd_Bes3_D0, Nev_tot, NBes3_D0, q2_inf_Bes3_D0, q2_sup_Bes3_D0, par_boot, sqr_dGamma_Bes3_D0, 0, p_cube);

  // BES3-D+
	Vckm_from_sqrdGamma_boot( Vcd_Bes3_Dp, Nev_tot, NBes3_Dp, q2_inf_Bes3_Dp, q2_sup_Bes3_Dp, par_boot, sqr_dGamma_Bes3_Dp, 1, p_cube);

  // BELLE
	Vckm_from_fpVckm_boot( Vcd_Belle, Nev_tot, NBelle, q2_Belle, par_boot, fpVcd_Belle, fplus_func);
  ...
  //sqr_dGamma_all
	append_six_arrays( sqr_dGamma_all, sqr_dGamma_BaBar, sqr_dGamma_Cleo_D0, sqr_dGamma_Cleo_Dp, sqr_dGamma_Bes3_D0, sqr_dGamma_Bes3_Dp, fpVcd_Belle, NBaBar, NCleo_D0, NCleo_Dp, NBes3_D0, NBes3_Dp, NBelle, 0);

  //sigma_sqr_dGamma_all
	append_six_arrays( sigma_sqr_dGamma_all, sigma_sqr_dGamma_BaBar, sigma_sqr_dGamma_Cleo_D0, sigma_sqr_dGamma_Cleo_Dp, sigma_sqr_dGamma_Bes3_D0, sigma_sqr_dGamma_Bes3_Dp, sigma_fpVcd_Belle, NBaBar, NCleo_D0, NCleo_Dp, NBes3_D0, NBes3_Dp, NBelle, 0);
  ...
  //// MATRICE DI COVARIANZA DOVUTA AI CONTRIBUTI DI RETICOLO PER Vcd
  
  int Size_blks[Nset_data] = { NBaBar, NCleo_D0, NCleo_Dp, NBes3_D0, NBes3_Dp, NBelle};
  
  double *Vcd_cov_lat=(double*)malloc(sizeof(double)*(Ntot)*(Ntot)), *Vcd_cov_lat_block=(double*)malloc(sizeof(double)*(Ntot)*(Ntot)), *var_Vckm=(double*)calloc( 1, sizeof(double));
  *var_Vckm = 0.0;
  
  covariance_syntetic_data( Vcd_cov_lat, Ntot, Nev_tot, Nev_an, *var_Vckm, Vcd_all, analysis_in, analysis_fin, clusterfile, 0);
  free(var_Vckm);

  //// COSTRUISCO LA MATRICE Vcd_cov_lat_block
  for(int i = 0; i < Ntot; i++){
    for(int j = 0; j < Ntot; j++){
      Vcd_cov_lat_block[i*Ntot+j] = Vcd_cov_lat[i*Ntot+j];
    }// j
  }// i

	block_matrix( Vcd_cov_lat_block, Ntot, Nset_data, Size_blks);
  /////////////////////////////////////////////////////////////


#if defined(NO_VCD_COV_LAT)
  ///// PRENDO SOLO LA DIAGONALE
  for(int i = 0; i < Ntot; i++){
    for(int j = 0; j < Ntot; j++){
      Vcd_cov_lat[i*Ntot+j] = 0.0;
    }// j
  }// i
#endif
  
#if defined(VCD_COV_LAT_DIAG)
  ///// PRENDO SOLO LA DIAGONALE
  for(int i = 0; i < Ntot; i++){
    for(int j = 0; j < Ntot; j++){
      if( i != j ){
	...
	block_matrix( Vcd_cov_lat, Ntot, Nset_data, Size_blks);
#endif  

  //printf("\n\nLATC Vcd_cov_lat \n\n");
  //print_matrix_for_mathematica( Ntot, Ntot, Vcd_cov_lat, 0);

  //// FINE MATRICE DI COVARIANZA DOVUTA AI CONTRIBUTI DI RETICOLO PER Vcd


  //////////////////////  MEDIA PESATA PER RICAVARE Vcd
  double *Vcd_cov=(double*)malloc(sizeof(double)*(Ntot)*(Ntot)), *Vcd_cov_sort=(double*)malloc(sizeof(double)*(Ntot)*(Ntot));
  double *Vcd_cov_exp=(double*)malloc(sizeof(double)*(Ntot)*(Ntot)); // La matrice viene calcolata evento per evento quando faccio il fit
  ...
  for(int ic = 0; ic < Ncut; ic++){
  	...
  	for(int iev = 1; iev <= Nev_tot; iev++){
  		...
  		//// MATRICE DI COVARIANZA DI Vcd DOVUTA AI CONTRIBUTI DELLE DELTA GAMMA SPERIMENTALI
  		for(int i = 0; i < Ntot; i++){
    		for(int j = 0; j < Ntot; j++){
					Vcd_cov_exp[i*Ntot+j] = cov_Vckm_exp_func( Vcd_all, sqr_dGamma_all, sigma_sqr_dGamma_all, cov_exp_sqr_dGamma, i, j, Nev_tot, iev, Ncov);
    		}// j
  		}// i  
    	...
    	for(int i = 0; i < Ntot; i++){
      	...
      	for(int j = 0; j < Ntot; j++){
					...
					Vcd_cov[i*Ntot+j] = Vcd_cov_lat_block[i*Ntot+j] + Vcd_cov_exp[i*Ntot+j];
	
					Vcd_cov_sort[i*Ntot+j] = Vcd_cov_lat[i*Ntot+j] + Vcd_cov_exp[i*Ntot+j];
			...
			sort_matrix( Vcd_cov_sort, Ntot, Ntot, q2_all, q2_all);

			//printf("\n\nWWWW Vcd_cov_sort \n\n");
			//print_matrix( Ntot, Ntot, Vcd_cov_sort, 1);
#endif  
			...
			for(int i = 0; i < Ntot; i++){
				for(int j = 0; j < Ntot; j++){
					...
					if( q2_all[i] > q2_cut[ic] || q2_all[j] > q2_cut[ic]){
						...
						if(i == j){
							Vcd_cov[i*Ntot+j] = 1.0;
						}else if(i != j){
							Vcd_cov[i*Ntot+j] = 0.0;
						}
			...
			for(int i = 0; i < Ntot; i++){
				for(int j = 0; j < Ntot; j++){
					...
					if( q2_all_sort[i] > q2_cut[ic] || q2_all_sort[j] > q2_cut[ic]){
						...
						if(i == j){
							Vcd_cov_sort[i*Ntot+j] = 1.0;
						}else if(i != j){
							Vcd_cov_sort[i*Ntot+j] = 0.0;
						}
			...
			Vcd_fit_BaBar[iev]      = Vckm_const_fit_with_cut( Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot,       0.0, Nsubgrp_1, iev, ic);
			Vcd_fit_Cleo_D0[iev]    = Vckm_const_fit_with_cut( Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_1, Nsubgrp_2, iev, ic);
			Vcd_fit_Cleo_Dp[iev]    = Vckm_const_fit_with_cut( Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_2, Nsubgrp_3, iev, ic);
			Vcd_fit_Bes3_D0[iev]    = Vckm_const_fit_with_cut( Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_3, Nsubgrp_4, iev, ic);
			Vcd_fit_Bes3_Dp[iev]    = Vckm_const_fit_with_cut( Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_4, Nsubgrp_5, iev, ic);
			Vcd_fit_Belle[iev]      = Vckm_const_fit_with_cut( Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_5, Nsubgrp_6, iev, ic);

			Vcd_fit_correlated[iev] = Vckm_const_fit_with_cut_corr( Vcd_all_sort, Vcd_cov_sort, q2_all_sort, q2_cut, Ntot, 0, Nbin_max, ic);
  ...
  for(int i = 0; i < Ntot; i++){
		...
		temp_sigma_Vcd_all[i] = sqrt(Vcd_cov[i+Ntot*i]);
	}// i
  ...
  double Vcd_single_corr,    sigma_Vcd_single_corr,    Xi2_corr,    Xi2_tot          = 0.0;
	...
	for(int i = 0; i < Ntot; i++){
		for(int j = 0; j < Ntot; j++){
			Vcd_cov_exp[i*Ntot+j] = cov_Vckm_exp_func( Vcd_all, sqr_dGamma_all, sigma_sqr_dGamma_all, cov_exp_sqr_dGamma, i, j, Nev_tot, 0, Ncov);
		}// j
	}// i
  ...
  for(int ic = 0; ic < Ncut; ic++){
		...
		/////////// MEDIA PESATA
		Vcd_single_BaBar   = Vckm_single_fit_with_cut( Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot,       0.0, Nsubgrp_1-1, ic);
		Vcd_single_Cleo_D0 = Vckm_single_fit_with_cut( Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_1, Nsubgrp_2-1, ic);
		Vcd_single_Cleo_Dp = Vckm_single_fit_with_cut( Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_2, Nsubgrp_3-1, ic);
		Vcd_single_Bes3_D0 = Vckm_single_fit_with_cut( Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_3, Nsubgrp_4-1, ic);
		Vcd_single_Bes3_Dp = Vckm_single_fit_with_cut( Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_4, Nsubgrp_5-1, ic);
		Vcd_single_Belle   = Vckm_single_fit_with_cut( Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_5, Nsubgrp_6-1, ic);

		Vcd_single_corr    = Vckm_single_fit_with_cut_corr( Vcd_all_sort, Vcd_cov_sort, q2_all_sort, q2_cut, Ntot, 0.0,  Nbin_max-1, ic);


		sigma_Vcd_single_BaBar   = sigma_Vckm_single_fit_with_cut( Vcd_cov, q2_all, q2_cut, Ntot,       0.0, Nsubgrp_1-1, ic);
		sigma_Vcd_single_Cleo_D0 = sigma_Vckm_single_fit_with_cut( Vcd_cov, q2_all, q2_cut, Ntot, Nsubgrp_1, Nsubgrp_2-1, ic);
		sigma_Vcd_single_Cleo_Dp = sigma_Vckm_single_fit_with_cut( Vcd_cov, q2_all, q2_cut, Ntot, Nsubgrp_2, Nsubgrp_3-1, ic);
		sigma_Vcd_single_Bes3_D0 = sigma_Vckm_single_fit_with_cut( Vcd_cov, q2_all, q2_cut, Ntot, Nsubgrp_3, Nsubgrp_4-1, ic);
		sigma_Vcd_single_Bes3_Dp = sigma_Vckm_single_fit_with_cut( Vcd_cov, q2_all, q2_cut, Ntot, Nsubgrp_4, Nsubgrp_5-1, ic);
		sigma_Vcd_single_Belle   = sigma_Vckm_single_fit_with_cut( Vcd_cov, q2_all, q2_cut, Ntot, Nsubgrp_5, Nsubgrp_6-1, ic);

		sigma_Vcd_single_corr    = sigma_Vckm_single_fit_with_cut( Vcd_cov_sort, q2_all_sort, q2_cut, Ntot, 0.0, Nbin_max-1, ic);

		/////////// CALCOLO I CHI2
		Xi2_BaBar   = Xi2_with_cut( Vcd_single_BaBar,   Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot,       0.0, Nsubgrp_1-1, ic);
		Xi2_Cleo_D0 = Xi2_with_cut( Vcd_single_Cleo_D0, Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_1, Nsubgrp_2-1, ic);
		Xi2_Cleo_Dp = Xi2_with_cut( Vcd_single_Cleo_Dp, Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_2, Nsubgrp_3-1, ic);
		Xi2_Bes3_D0 = Xi2_with_cut( Vcd_single_Bes3_D0, Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_3, Nsubgrp_4-1, ic);
		Xi2_Bes3_Dp = Xi2_with_cut( Vcd_single_Bes3_Dp, Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_4, Nsubgrp_5-1, ic);
		Xi2_Belle   = Xi2_with_cut( Vcd_single_Belle,   Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_5, Nsubgrp_6-1, ic);

		Xi2_corr    = Xi2_with_cut_corr( Vcd_single_corr, Vcd_all_sort, Vcd_cov_sort, q2_all_sort, q2_cut, Ntot, 0.0, Nbin_max-1, ic);
		...
		if(ic == Ncut-1){
			Xi2_part_BaBar   = (Xi2_with_cut( Vcd_single_corr, Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot,       0.0, Nsubgrp_1-1, ic)*  (NBaBar-1.0))/(Nbin_max-1.0);
			Xi2_part_Cleo_D0 = (Xi2_with_cut( Vcd_single_corr, Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_1, Nsubgrp_2-1, ic)*(NCleo_D0-1.0))/(Nbin_max-1.0);
			Xi2_part_Cleo_Dp = (Xi2_with_cut( Vcd_single_corr, Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_2, Nsubgrp_3-1, ic)*(NCleo_Dp-1.0))/(Nbin_max-1.0);
			Xi2_part_Bes3_D0 = (Xi2_with_cut( Vcd_single_corr, Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_3, Nsubgrp_4-1, ic)*(NBes3_D0-1.0))/(Nbin_max-1.0);
			Xi2_part_Bes3_Dp = (Xi2_with_cut( Vcd_single_corr, Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_4, Nsubgrp_5-1, ic)*(NBes3_Dp-1.0))/(Nbin_max-1.0);

		#if defined(BELLE)
			Xi2_part_Belle   = (Xi2_with_cut( Vcd_single_corr, Vcd_all, Vcd_cov, q2_all, q2_cut, Ntot, Nev_tot, Nsubgrp_5, Nsubgrp_6-1, ic)*(NBelle-1.0))/(Nbin_max-1.0);
		#endif
		#if defined(NO_BELLE)
			Xi2_part_Belle   = 0.0;
		#endif
...
void Vckm_from_sqrdGamma_boot( double* Vckm, int Nev, int Nexp, double* q2_inf, double* q2_sup, double par_boot[Nev_tot+1][npar], double* sqr_dGamma, int isospin, double (*p_cube)( double, void*)){
  ...
  for(int i = 0; i < Nexp; i++){
    ...
    for(int iev = 1; iev <= Nev; iev++){
      ...
      Vckm[i*(Nev+1)+iev] = (C*sqr_dGamma[i])/sqrt(int_result);
...
void Vckm_from_fpVckm_boot( double* Vckm, int Nev, int Nexp, double* q2, double par_boot[Nev_tot+1][npar], double* fpVckm, double (*fplus_func)( double, void*)){
